 ...
 * be applied before MapLibre sees it. The result is the patched style JSON, or `null` when anything
 * goes wrong (offline, non-JSON body, relative URLs) — the caller then points MapLibre at the URL
 * directly and only loses the patches. The URL is a parameter because a dedicated dark style can
...
class MapStyleLoader(private val client: OkHttpClient) {
    ...
    suspend fun load(styleUrl: String, night: Boolean): String? {
        val body =
        ...
        return try {
            ...
            if (night) NightStylePatch.apply(patched) else patched
    ...
    private suspend fun fetch(styleUrl: String): String? =
